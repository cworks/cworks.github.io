...
module Jekyll
  ...
  class PhotoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @filename
        "<a href=\"#{p.path_for(@filename)}\" title=\"#{@title}\"><img src=\"#{p.thumb_for(@filename,@thumb)}\" alt=\"#{@title}\" /></a>"
      else
    ...
    def render(context)
      ...
      lines.each do |line|
        if /(?<filename>[^\[\]:]+)(?:\[(?<thumb>\S*)\])?(?::(?<title>.*))?/ =~ line
          list << "<li><a href=\"#{p.path_for(filename)}\" rel=\"gallery-#{md5}\" title=\"#{title.strip}\">"
